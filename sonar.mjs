...
(async () => {
  ...
  // track requests so we can abort them if they collide
  var requests = [];
  var count = 0;
  ...
  function arrangeFeatures() {
    ...
    displayBins(bins); // toggle this comment to enable/disable
    ...
    for (var x = 0; x < bins.length; x++) {
      ...
      if (bins[x].length) { // if it has any entries
        ...
        // apply equal loudness factor to tone down the high end
